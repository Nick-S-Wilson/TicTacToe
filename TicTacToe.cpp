...
bool state(char cb[][DIM], int row, int col, char x)
{
	...
	/* We declare four variables count1, count2, count3, count4 to represent the occurrence number of
	x in row number row, column number col, in the main diagonal, and in the opposite diagonal.
	If after calculation, count1, count2, count3 or count4 equals to DIM, return true(that is, x wins). */
	...
	for (int i = 0; i<DIM; ++i)
	{
		...
		if (chessboard[i][DIM - 1 - i] == x)
		{
			count4 += 1;
		}
		if (count1 == 3 || count2 == 3 || count3 == 3 || count4 == 3)
		{
			return 1; // returns a win and ends the game
		}
